...
import { EmployeeSectionPopupComponent } from '../employee-section-popup/employee-section-popup.component';


@Component({
  selector: 'app-employee-section-list',
  templateUrl: './employee-section-list.component.html',
  styleUrls: ['./employee-section-list.component.scss']
})
export class EmployeeSectionListComponent implements OnInit {
  ...
  constructor(
    ...
    openDialog(): void {
      this.dialog.open(EmployeeSectionPopupComponent, {
        width: '500px',
      });
    }
    ...
    getImportEmployeesDivisions(): void {
      ...
      this.acsServerService.getEmployeesDivisions(params).subscribe((response) => {
        ...
        if (response?.status == "Error") {
          const dialogRef = this.dialog.open(EmployeeSectionPopupComponent, {
